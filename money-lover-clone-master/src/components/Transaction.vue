<template>
    <div>
        ...
        <divider/>
        <div class="w-100 h-24 bg-gray-400 shadow-inner">
        ...
        <div class="w-100 flex">
            <div class="w-full bg-gray-400 hover:bg-gray-600 h-10 text-gray-700 hover:text-gray-200 cursor-pointer font-semibold flex justify-center items-center">
        ...
        <div class="w-100 flex flex-col">
            <div class="w-full bg-white flex flex-col" v-for=" ( transaction, index ) in transactions" :key="index">
                
                <div class="w-full flex flex-col px-12 pt-12 pb-6">
                    <div class="w-full flex justify-between text-lg font-semibold">
                        <div class="time" v-for="( dayTrans, index ) in transaction.date" :key="index">
                            <span class="day">{{ dayTrans.day }}</span>/<span class="month">{{ dayTrans.month }}</span>/<span class="year">{{ dayTrans.year }}</span>, <span class="weekday">{{ dayTrans.date }}</span>
                        </div>
                        <div class="cash">
                            {{ transaction.total }}
                        </div>
                    ...
                    <div class="w-full flex justify-between py-5 px-6 mb-5 border-2" v-for=" ( transDetails, index ) in  transaction.trans" :key="index">
